"""
gnss_simulate_hypothesis_det_and_ident_line.py

HPC GNSS detection-and-identification simulation, LINE version.

Difference vs. the grid version (gnss_simulate_hypothesis_det_and_ident.py):
    * For q_i = 2 hypotheses the bias is sampled on a LINE, b = [bi, bi],
      NOT on a 2-D grid. q_i = 1 is unchanged (b = [bi]). Both therefore
      produce 1-D IR arrays over a single b-line.

Parallelization:
    * One SLURM task per hypothesis (--hypt).
    * Within the task, the N_sims independent Monte Carlo replications are
      distributed across CPUs with multiprocessing.Pool: one worker runs
      ONE full replication, i.e. it draws its own fresh y-vector(s) and
      sweeps ALL b-values on the line. The parent then averages the
      per-replication IR and reports the across-replication std.
    * Because q=2 is now a line (same cost as q=1), there is no longer a
      q=2/q=1 cost imbalance, so per-replication granularity is well
      balanced and the (more expensive) b-grid Pool is unnecessary.

A fresh y-vector is drawn for every replication (and, within a replication,
freshly per b-value), via independent SeedSequences derived from
(base_seed, hypt, i_sim).

Usage
-----
    python gnss_simulate_hypothesis_det_and_ident_line.py --hypt <int> [opts]
...
All IR values are CONDITIONAL on the hypothesis (no prior PH0/PHi multiplied
in). Multiply by the hypothesis probability in post-processing to get the
contribution to the total integrity risk.
"""
...
# ============================================================
# Test statistics + identification (identical to the non-parallel scripts)
# ============================================================
...
def compute_identified_x(y_samples, argmax, max_test, geom):
    """Identified x at the n_UP component for each sample.
    Detection (max_test >= 1) → constrained estimator Abar_plus_list[argmax];
...
def _init_worker(geom, hypt, al_bx_sq, b_values, nr_samples, base_seed):
    global _GEOM, _HYPT, _HYPT_LOCAL, _AL_BX_SQ, _B_VALUES, _NR_SAMPLES, _BASE_SEED
    _GEOM = geom
    _HYPT = hypt
...
def _worker_run_sim_H0(i_sim):
    ...
    # Fresh, independent y-vector for this replication
    ...
    alpha_SS = float(np.mean(max_SS > 1))
    alpha_Tq = float(np.mean(max_Tq > 1))
    ...
    IR_SS = np.sum(sq_SS[None, :] > AL_Bx_sq[:, None], axis=1) / nr_samples
    IR_Tq = np.sum(sq_Tq[None, :] > AL_Bx_sq[:, None], axis=1) / nr_samples
...
def _worker_run_sim_Hi(i_sim):
    ...
    per b-value. Returns conditional IR|Hi(b) per AL_Bx for every b in the
    line, for this replication."""
    geom = _GEOM
    hypt = _HYPT_LOCAL
    AL_Bx_sq = _AL_BX_SQ
    b_values = _B_VALUES
    ...
    # One rng for this replication; advanced sequentially across b-values so
    # every (i_sim, b) gets its own fresh, reproducible noise.
    ...
    return IR_SS, IR_Tq  # each (n_AL, n_b)
...
def simulate_H0(geom, AL_Bx_list, N_sims, nr_samples, base_seed, n_workers):
    ...
    with Pool(
        ...
        initargs=(geom, 0, AL_Bx_sq, np.empty(0), nr_samples, base_seed),
    ) as pool:
...
def simulate_Hi(geom, hypt_local, AL_Bx_list, N_sims, nr_samples, base_seed,
    ...
    with Pool(
        processes=n_workers,
        initializer=_init_worker,
        initargs=(geom, hypt_local + 1, AL_Bx_sq, b_values, nr_samples, base_seed),
    ) as pool:
        for i_sim, (ir_SS, ir_Tq) in enumerate(
...
def main():
    ...
    p.add_argument('--geom-seed', type=int, default=1,
                   help='Seed for satellite selection (must match across all '
                        'hypothesis tasks of a given run).')
    ...
    p.add_argument('--out-base', type=str,
                   default='/home/bvannoort/CH5/Data_GNSS_example/b_sim_1D/det_and_ident')
    ...
    print(f"[gnss_simulate_hypothesis_det_and_ident_line] hypt={args.hypt} "
