...
if __name__ == "__main__":
    ...
    fig = plt.figure(figsize=(16.6, 9.0))
    gs = GridSpec(
        2, 6, figure=fig, width_ratios=[20, 1, 20, 1, 20, 1], height_ratios=[1, 1]
    )
    colorbars = []

    fig_sigma = plt.figure(figsize=(16.6,9.0))
    gs_sigma = GridSpec(
    ...
    fig_flat, ax_zoom = plt.subplots(3, 1, figsize=(10, 8), sharex=False)
    ...
    for i_col, type_of_ci_matrix in enumerate(types_of_ci_matrices):
        ...
        figSavingDir = r'C:\Users\bgvannoort\Documents\Dissertation\Chapter 5 SS vs GLR\Figures\binary_example\det_and_ident' 

        if type_of_ci_matrix == "outlier":
            savingDir = os.path.join(savingDir, "2 Outliers", "Det+Ident", "Data")
        elif type_of_ci_matrix == "linearly_dependent":
            savingDir = os.path.join(
                savingDir, "Ci-almost-lin-dependent", "Det+Ident", "Data"
            )
        elif type_of_ci_matrix == "almost_in_A":
            savingDir = os.path.join(
                savingDir, "Ci-columns-almost-in-A", "Det+Ident", "AL=15", "Data"
            )
        ...
        conversion_for_sigma_std = 1e-2 # to convert the scale of the colorbar to be more visible for the sim STD. 
        ...
        b1grid_fine, b2grid_fine = np.meshgrid(
            np.linspace(np.min(b1grid), np.max(b1grid), 100),
            np.linspace(np.min(b2grid), np.max(b2grid), 100),
        )
        ...
        cb.ax.text(1.35, 0.98, s, transform=cb.ax.transAxes,
        ...
        cb_sigma.ax.text(1.35, 0.98, s_sigma, transform=cb_sigma.ax.transAxes,
                         ha='left', va='top', fontsize=16)
        

        IRS_to_plot.append([IR_grid_Tq[:, 0], IR_grid_SS[:, 0]])

        if type_of_ci_matrix == "linearly_dependent":
            diff_IR_SS_minus_Tq_lin_dependent = IR_grid_SS[:, 0].reshape(
                len(b1grid), len(b2grid)
            ) - IR_grid_Tq[:, 0].reshape(len(b1grid), len(b2grid))
        ...
        if type_of_ci_matrix == "outlier":
            x1, x2, y1, y2 = 0, 1500, -0.0, 0.007
        elif type_of_ci_matrix == "linearly_dependent":
            x1, x2, y1, y2 = 500, 1500, 0, 0.017
        elif type_of_ci_matrix == "almost_in_A":
            # x1, x2, y1, y2 =  2300, 2500, -0.005, 0.05
            x1, x2, y1, y2 = 1500, 2000, -0.005, 0.1
        else:
            x1, x2, y1, y2 = 2300, 2500, -0.005, 0.05
        ...
        ax_zoom2.plot(perc_diff, color="green", linestyle="-", label="Diff(\%)", linewidth=2.5)
    ...
    #     ax_all.plot(np.sort(diff_curve), color=zz[i], label=label_names[i])
    #     # ax_all.plot(np.sort(IRS_all_GLR[i]), color=zz[i], linestyle='-.')
    #     # ax_all.plot(np.sort(IRS_all_SS[i]), color=zz[i], linestyle='dotted')

    #     ax_all.plot()

    #     ax_all_perc.plot(np.sort(all_diff_perc[i]), color=zz[i], linestyle="--")
    
    
    ax_all_perc.set_ylabel("Rel. diff [\%]", fontsize=18 )
    # ax_all.set_title(r"DIA $\mathbb{P}_{\mathcal{F}}$ and relative differences as a function of $b$")
